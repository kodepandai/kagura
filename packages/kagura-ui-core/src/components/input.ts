...
const createStatus = (
  status: InputStatus,
  statuses?: { invalid?: CSSRuleObject; disabled?: CSSRuleObject },
) => {
  return {
...
export const createInputStyle = (input?: Partial<PresetInput>) => {
  ...
  const inputVariants = variants.reduce(
    (collect, variant) => ({
      ...collect,
      ...createVariant(variant, input?.variants),
    }),
    {},
  );
...
export default (theme: Theme) => {
  ...
  Object.keys(theme("kagura")).map((scope) => {
    let input = theme<Preset["components"]>(`kagura.${scope}.components`)
      ?.input;
